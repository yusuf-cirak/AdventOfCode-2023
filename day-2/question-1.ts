...
function getSumOfPossibleGameNumbers(gameNumbers: number[]) {
  return gameNumbers.reduce((sum, currentGameNumber) => {
    return sum + currentGameNumber;
  }, 0);
}

function getPossibleGameNumbers(gameInfos: GameInfo[]) {
  return gameInfos.reduce((possibleGameNumbers: number[], currentGameInfo) => {
    const gameSetInfo: GameSetInfo = Object.values(currentGameInfo)[0];

    let isGamePossible = true;

    for (const setInfo of Object.values(gameSetInfo)) {
      if (!isSetPossible(setInfo)) {
    ...
    if (isGamePossible) {
      const gameNumber = Number(Object.keys(currentGameInfo)[0]);
      possibleGameNumbers.push(gameNumber);
    }

    return possibleGameNumbers;
  }, []);
}

function isSetPossible(setInfo: GameSetInfo) {
  return Object.values(setInfo)[0].reduce(
    (isSetPossible: boolean, currentColorInfo) => {
      const { color, colorCount } = currentColorInfo;

      return isSetPossible && colorCount <= maxCubeCount[color];
    },
    true,
  );
}
...
const gameInfos = getGameInfos(lines);

const possibleGameNumbers = getPossibleGameNumbers(gameInfos);

// console.log(maxCubeCount);
// console.log(possibleGameNumbers);

const sum = getSumOfPossibleGameNumbers(possibleGameNumbers);
