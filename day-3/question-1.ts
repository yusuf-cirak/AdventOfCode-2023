...
function getSumOfNumberRatios(lines: string[]) {
  ...
  const numberList = [...numbers.values()];

  console.log(numberList);

  return numberList.reduce((prev, next) => prev + next, 0);
}
...
function findNumbersWithSymbols(lines: string[]) {
  ...
  lines.forEach((line, lineIndex) => {
    for (let index = 0; index < line.length; index++) {
      ...
      // First occurence to a number, find rest of the number
      if (!isNaN(Number(character))) {
        const numberStartIndex = index;
        const numberString = getNumberString(line, index);
        console.log(`Found number: ${numberString}`);
        const numberEndIndex = numberStartIndex + numberString.length - 1;

        // Check if number is followed by a symbol
        const isFollowedBySymbol = hasGearNumbersFromOwnLine(
          line,
          numberStartIndex,
          numberEndIndex,
        );

        if (isFollowedBySymbol) {
...
function hasGearNumbersFromOwnLine(
  line: string,
  numberStartIndex: number,
  numberEndIndex: number,
) {
  const characterBeforeNumber = line[numberStartIndex - 1];
  const characterAfterNumber = line[numberEndIndex + 1];

  return (
    symbols.includes(characterBeforeNumber) ||
    symbols.includes(characterAfterNumber)
  );
}

function isGearNumberFromNeighbor(
  ...
  const startIndex = Math.max(0, numberStartIndex - 1);
  const endIndex = Math.min(neighborLine.length - 1, numberEndIndex + 1);
