...
function getSumOfNumberRatios(lines: string[]) {
  const numbers = findNumbersWithSymbols(lines);
...
function findNumbersWithSymbols(lines: string[]) {
  ...
  lines.forEach((line, lineIndex) => {
    for (let index = 0; index < line.length; index++) {
      ...
      if (symbols.includes(character)) {
        ...
        // Check if number is followed by a symbol
        const gearNumbersFromOwnLine = hasNumbersBeforeAndAfterSymbolInSameLine(
          line,
          symbolIndex,
        );

        if (gearNumbersFromOwnLine) {
          const numberArr = getNumbersFromSameLine(line, symbolIndex);
          console.log(numberArr);
          const multiply = numberArr.reduce((prev, next) => prev * next, 1);
          numbers.push(multiply);
          continue;
        }

        // Check if number is followed by a symbol in the next line or previous line
        const isFollowedByNeighborSymbol =
          hasNumbersBeforeAndAfterSymbolInNeighborLine(
            lines[lineIndex - 1],
            symbolIndex,
          ) &&
          hasNumbersBeforeAndAfterSymbolInNeighborLine(
        ...
        if (isFollowedByNeighborSymbol) {
          const numberArr = [
            getNumberFromNeighborLine(lines[lineIndex - 1], symbolIndex),
            getNumberFromNeighborLine(lines[lineIndex + 1], symbolIndex),
          ];
          const multiply = numberArr.reduce((prev, next) => prev * next, 1);
...
function getNumberFromLineReversed(line: string, startingIndex: number) {
  let number = "";
...
function getNumbersFromSameLine(line: string, index: number) {
  const firstNumber = getNumberFromLineReversed(line, index)
    .split("")
    .reverse()
    .join("");
  const secondNumber = getNumberString(line, index + 1);

  return [Number(firstNumber), Number(secondNumber)];
}

function getNumberFromNeighborLine(
  line: string,
  index: number,
  reverse = false,
) {
  if (reverse) {
    return Number(getNumberFromLineReversed(line, index));
  }
  let number = "";
  ...
  if (
    number === "" ||
    (!isNaN(Number(number)) && !isNaN(Number(line[index - 1])))
  ) {
    number = getNumberFromLineReversed(line, startIndex);
    reverse = true;
  }

  number = reverse ? number.split("").reverse().join("") : number;

  console.log(`Found number: ${number}`);
  return Number(number);
}

function hasNumbersBeforeAndAfterSymbolInSameLine(line: string, index: number) {
  const characterBefore = line[index - 1];
  const characterAfter = line[index + 1];

  return !isNaN(Number(characterBefore)) && !isNaN(Number(characterAfter));
}

function hasNumbersBeforeAndAfterSymbolInNeighborLine(
  neighborLine: string | undefined,
  symbolIndex: number,
) {
  if (neighborLine === undefined) {
    return false;
  }
  let hasNumbersBeforeAndAfterSymbol = false;

  const startIndex = Math.max(0, symbolIndex - 1);
  const endIndex = Math.min(neighborLine.length - 1, symbolIndex + 1);

  for (let index = startIndex; index <= endIndex; index++) {
    const character = neighborLine[index];

    if (!isNaN(Number(character))) {
      hasNumbersBeforeAndAfterSymbol = true;
      break;
    }
  }

  return hasNumbersBeforeAndAfterSymbol;
}

function getNumberString(input: string, startIndex: number) {
  let numberStr = input[startIndex];
...
const sum = getSumOfNumberRatios(lines);
console.log(sum);
